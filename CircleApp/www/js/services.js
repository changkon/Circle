angular.module('starter.services', ['ngCordova'])
    ...
    .service('event', ['$rootScope', '$cordovaSms', '$ionicPopup', function ($rootScope, $cordovaSms, $ionicPopup) {
        ...
        this.getEventStatusString = function(value) {
            if (value === EventStatus.HOST) {
                return "Host";
            } else if (value === EventStatus.ATTENDING) {
                return "Attending";
            } else if (value === EventStatus.NOTGOING) {
                return "NotGoing";
            } else if (value === EventStatus.PENDING) {
                return "Pending";
            }
        };
...
// http://jonsamwell.com/url-route-authorization-and-security-in-angular/
    .service('authorization', ['$rootScope', 'event', function ($rootScope, event) {

        var PermissionCheckLevels = Object.freeze({
            AtLeastOne: "AtLeastOne",
            CombinationRequired: "CombinationRequired"
        });

        var authorize = function (loginRequired, requiredPermissions, permissionCheckType, userStatus) {
            var authorized = true;
            ...
            // set permissionCheckType or set to default
            permissionCheckType = permissionCheckType || PermissionCheckLevels.AtLeastOne;

            if (loginRequired === true && userStatus === undefined) {
                authorized = false;
            } else if ((loginRequired === true && userStatus !== undefined) && (requiredPermissions === undefined || requiredPermissions.length === 0)) {
                // Login is required but no specific permissions are specified
                authorized = true;
            } else if (requiredPermissions) {
                loweredPermissions = [];
                loweredPermissions.push(userStatus);

                for (i = 0; i < requiredPermissions.length; i++) {
                    permission = requiredPermissions[i];
                  
                    // Checks to see if the current permission is available
                    if (permissionCheckType === PermissionCheckLevels.CombinationRequired) {
                    ...
                    } else if (permissionCheckType === PermissionCheckLevels.AtLeastOne) {
                        // check permission is available
                        hasPermission = loweredPermissions.indexOf(permission) > -1;
                      
                        // it has one of the permissions, break
                        if (hasPermission) {
                            break;
                        }
                    }
        ...
        this.authorized = function(loginRequired, requiredPermissions, permissionCheckType, userStatus) {
            return authorize(loginRequired, requiredPermissions, permissionCheckType, userStatus);
        };
    }]);
